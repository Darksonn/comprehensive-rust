...
## Example: PCI Driver with MMIO

Below is a PCI driver that maps **BAR 0** and performs basic read and write operations during `probe`.

```rust,ignore
// SPDX-License-Identifier: GPL-2.0
//! PCI driver with MMIO register access.
...
// Driver data holds the mapped BAR, bound to the device lifetime
struct TestPciData<'bound> {
    _bar: pci::Bar<'bound, { regs::END }>,
}
...
impl pci::Driver for TestPciDriver {
    ...
    fn probe<'bound>(
        ...
        // We return a Result from the closure, which implements PinInit
        let init = move || {
            ...
            // 3. Read and write registers directly:
...
kernel::module_pci_driver! {
    type: TestPciDriver,
    name: "rust_driver_pci_testdev_mmio",
    authors: ["Your Name"],
    description: "PCI testdev MMIO driver",
    license: "GPL v2",
}
```
